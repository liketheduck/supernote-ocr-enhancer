...
def export_note_to_logseq_flat(
    ...
    try:
        ...
        if pdf_source_path and pdf_source_path.exists():
            ...
            import shutil
            logger.debug(f"PDF source path: {pdf_source_path}")
            logger.debug(f"PDF asset path: {pdf_asset_path}")
            logger.debug(f"PDF source exists: {pdf_source_path.exists()}")
            if pdf_source_path.exists():
                logger.debug(f"PDF source size: {pdf_source_path.stat().st_size} bytes")
                shutil.copy2(pdf_source_path, pdf_asset_path)
                logger.debug(f"Copied PDF to Logseq assets: {pdf_asset_path}")
                logger.debug(f"PDF asset exists after copy: {pdf_asset_path.exists()}")
            else:
                logger.warning(f"PDF source path does not exist: {pdf_source_path}")
        else:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if temp_pdf and temp_pdf.exists():
                    ...
                    logger.debug(f"Copied generated PDF to Logseq assets: {pdf_asset_path}")
                else:
